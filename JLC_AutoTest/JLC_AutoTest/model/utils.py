...
import  pymysql
import   random
from JLC_AutoTest.common.settings import MY_SQL_INFO,REDIS_INFO,BASE_PATH,SNB_MY_SQL_INFO,OLD_PHONE
import redis
import codecs,os
from datetime import date,timedelta
from JLC_AutoTest.common.settings import REPORT_PATH
...
def create_bank(time):
    ...
    list = []
    for i in range(time):
        code = prelist[0] + "".join(random.choice("0123456789") for i in range(8))
        list.append(code)
    return list
...
#发邮件
import  yagmail
def send_mail(reult_path):

    username='709462978@qq.com'
    password='oyujmmcmoubqbfeh'#生成授权码，qq、163、126都是授权码
    ...
    m.send(to=to,cc=cc,subject='拾年宝UI自动化报告',
           contents='拾年宝UI自动化报告如附件所示',attachments=reult_path)
# send_mail(r'/Users/jlc/Downloads/JLC_AutoTest/JLC_AutoTest/files/report/2019-01-07 15_15_04result.html')
